(function (root) {
  ...
  function resultText(result) {
    ...
    const b = result.book || {};
    [["书名", b.title], ["作者", b.author], ["出版社", b.publisher], ["出版时间", b.pubdate], ["ISBN", b.isbn]].forEach(([k, v]) => { if (v) lines.push(`${k}：${v}`); });
  ...
  function card(result, compact = false) {
    ...
    copy.addEventListener("click", async () => { try { await navigator.clipboard.writeText(resultText(result)); copy.textContent = "已复制"; setTimeout(() => copy.textContent = "复制", 1400); } catch { copy.textContent = "复制失败"; } });
